                  ...
                  <tr v-if="is_file_uploaded && iscolumnsnamechanged">
                    <table class="table">
                      ...
                      <tr>
                        ...
                        >Save Metrics</button>
                        <!-- </form> -->
                        <!-- <div id="collapseExamples__2" class="col-sm-8 collapse">
  ...
  mounted() {
    ...
    axios
      .get(api_calls.bi_data_sheet_details(), {
        ...
        if(response.data.data_sources.length > 0) {
        this.is_dataset_editable = true;
        this.file_label = response.data.data_sources[0].file_name;
        $("#inputGroupFile01").prop("disabled", true);
        $("#dataset_name").prop("disabled", true);
        this.dataset_name = response.data.data_sources[0].data_name;
        $("#upload_file").hide();
        // console.log(response.data.data_sources[0].worksheet_details.column_details);


        for (let [key, value] of Object.entries(response.data.data_sources[0].worksheet_details.column_rename_map)) {
                this.columns.push({
                  name: key,
                  newname: value,
                });
        ...
        this.is_file_uploaded = true;
        var metrics_len = response.data.data_sources[0].worksheet_details.metrics.length; 
        for(let i = 0; i < metrics_len ; i++) {
          this.metrics.push({
            name: response.data.data_sources[0].worksheet_details.metrics[i].name,
            column: response.data.data_sources[0].worksheet_details.metrics[i].column,
            operation: response.data.data_sources[0].worksheet_details.metrics[i].operation,
          })
        }
        // debugger;
        // console.log(this.metrics);
      ...
      var object = this.columns.reduce(
            ...
            for (let [key, value] of Object.entries(response.data.data_sources[0].worksheet_details.column_details)) {
              for (let i = 0; i < this.columns.length; i++) {
