              ...
              <table class="table">
                <thead>
                  <tr>
                    ...
                    "
                    >
                      {{tlt('product_config_cross_prod_name_label')}}
                    </th>
                  </tr>
                </thead>
                <tbody>
        ...
        <div class="col-sm-8" style="overflow :visible;">
          ...
          >
            ...
            <div class="d-flex px-3 pt-3">
              <multiselect
                v-model="current_product.cross_sell_products_list"
                :options="cross_sell_products"
                :multiple="true"
                ...
                label="title"
                track-by="id"
                :max="limit_cross_sell"
              >
                <template slot="selection" slot-scope="{ values, isOpen }"
                  ><span
                    class="multiselect__single"
                    v-if="values.length && !isOpen"
                    >{{ values.length }} {{tlt('cross_sell_product_selected_msg')}}</span
                  ></template
                >
                <template slot="maxElements"
                  ><span class="multiselect_single"
                    >{{tlt('cross_sell_max_of_msg1')}} {{ tlt('cross_sell_max_of_msg2') }} {{tlt('cross_sell_max_of_msg3')}}</span
                  ></template
                >
                <template slot="tag">{{ "" }}</template>
              </multiselect>
            </div>
            <p v-show="current_product.cross_sell_products_list.length > limit_cross_sell" class="p-3">{{tlt('cross_sell_max_of_product_msg1')}} {{ tlt('cross_sell_max_of_product_msg2') }} {{tlt('cross_sell_max_of_product_msg3')}}</p>
            <div class="mt-5 p-3">
              <h5>{{tlt('cross_sell_max_of_product_msg4')}} {{ current_product.title }}</h5>
              <div style="border-bottom: 1px solid #e3e2e5"></div>
              <div
                v-for="(product,
                index) in current_product.cross_sell_products_list"
                :key="index"
...
export default {
  ...
  mounted() {
    ...
    this.limit_crossell_products();
    this.$root.$on("ProductsCrosssellPageDisabled", (data) =>{
      if(data===true){
  ...
  methods: {
    save_product_details() {
      // var myJSON = JSON.stringify(this.product_data_list);
      // console.log("Cross sell Produccccts :",myJSON);
      if(!this.disable_btn_expired){
      Swal.fire({
        title: this.tlt('cross_sell_swal_msg'),
        text: this.tlt('cross_sell_save_product_msg'),
        type: "question",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        confirmButtonText: this.tlt('cross_sell_swal_confirm_msg')
      }).then(result => {
        if (result.value) {
          this.spinnerOn = true;
          axios
            ...
            .then(response => {
              ...
              if (response.data.message.MSG_CODE == this.api_status_code.DATA_AVAILABLE_MODELS.MSG_CODE) {
                Swal({
                  type: "success",
                  text: this.tlt('cross_sell_data_added_confirm_msg'),
                  timer: 2500
                }).then(result => {
                  this.upsell_crosssell_products_data();
                });
    ...
    //           for (var i in temp) {
    //             //create a new array object new_products_is_any
    //             this.new_product_data_list.push({
    //               id: temp[i].id,
    //               title: temp[i].name,
    //               cross_sell_products_list: []
    //             });
    //             this.cross_sell_products.push({
    //               id: temp[i].id,
    //               title: temp[i].name
    //             });
    //           }
    ...
    //           }
    //         }
    //         // console.log("new product data", this.new_product_data_list);
    //         // this.spinnerOn = false;
    //         this.upsell_crosssell_products_data();
    ...
    upsell_crosssell_products_data() {
      ...
      axios
        ...
        .then(response => {
          if (response.data.message.MSG_CODE == this.api_status_code.DATA_AVAILABLE_MODELS.MSG_CODE) {
            // console.log(response.data.cross_sell);
            var temp = response.data.data.response_data.cross_sell;
            for (var i in temp) {
              this.product_data_list.push({
                id: temp[i].id,
                title: temp[i].title,
                cross_sell_products_list: temp[i].cross_sell_products_list ? temp[i].cross_sell_products_list : [] 
              });
              this.cross_sell_products.push({
                  id: temp[i].id,
                  title: temp[i].title
      ...
      axios
        ...
        .then(response => {
          if (
            ...
            if(this.retail_web_framework == "woocommerce"){
              this.limit_cross_sell = response.data.data.woocommerce_details.limit_up_sell;
            }
            else if(this.retail_web_framework == "shopify"){
              this.limit_cross_sell = response.data.data.shopify_details.limit_up_sell;
            }
            ...
            toastr.error(this.tlt('cross_sell_internal_server_alert'));
          }else{
        ...
        .catch(e => {
          this.spinnerOn = false;
          toastr.error(this.tlt('cross_sell_error_occurred_alert'));
          if (
            e.response.status === 410 ||
            e.response.status === 440 ||
            e.response.status === 409
    ...
    remove_button_product_upsell_name(index) {
      this.current_product.cross_sell_products_list.splice(index, 1);
    },
