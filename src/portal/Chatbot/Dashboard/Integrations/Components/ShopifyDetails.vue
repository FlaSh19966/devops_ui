<template>
  <div :style="setup_css" class="dash-modal">
    <div class="modal-head align-items-center m-0">
      ...
      <h3 style="text-align:center" v-if="from_setup == false">{{tlt('shopify_setting_header')}}</h3>
      <button
    ...
    <div class="modal-head d-flex align-items-center m-0">
      <h5 v-if="shopify_details_saved" style="width:100%">
        {{tlt('shopify_setting_sync_status_label')}}:
        <strong style="color: #273679">
          {{ shopify_sync_status }}
          <i
            class="fa fa-spinner fa-spin"
            v-show="shopify_sync_status === 'In-Progress'"
          ></i>
        </strong>

        <!-- <span
                    v-html="refresh_icon_svg"
                    @click="shopify_sync_current_step += 1"
                    style="float: right"
                    id="refresh_span"
                  ></span> -->
    ...
    <div class="slide-right" v-if="shopify_details_saved">
      <h5 v-show="shopify_sync_status === 'In-Progress' && from_setup"
        style="font-weight: 500;">
        {{tlt('shopify_setting_setup_process_label')}}.
      </h5>
    </div>
    <div class="progress-sync" v-if="shopify_details_saved">
      <step-progress
        :steps="shopify_sync_steps"
        :current-step="shopify_sync_current_step"
    ...
    <h5>
      {{ shopify_store_details_text }}
      <!-- <span
        @click="edit_shopify_details"
        v-html="pencil_svg"
        class="ml-auto"
        style="cursor: pointer; float: right"
        title="Edit Details"
      ></span> -->
      <!-- v-if="!shopify_details_saved && !is_shopify_public_app_for_company"  -->
      <span @click="shopify_help" 
    ...
    <ol class="pl-0 mt-2">
      <form @submit.prevent="submit_shopify_form">
        <div class="form-group mb-2">
          <label> {{tlt('shopify_setting_domain_name_label')}}: </label>
          <!-- <label> Shop Primary Domain Name (In the format: your-store-name.myshopify.com) For reference click <a href="https://help.shopify.com/en/manual/online-store/os/domains/domains-terminology#primary-domain" target="_blank">here</a> </label> -->
          <input
            class="form-control"
            type="text"
            v-model="shopify_shop_name"
            :disabled="(shopify_details_saved || is_shopify_public_app_for_company) || !from_setup"
            required
          />
        </div>

        <div class="form-group mb-2" v-if="!is_shopify_public_app_for_company">
          <label> {{tlt('shopify_setting_api_key_label')}}: </label>
          <input
            class="form-control"
            type="text"
            v-model="shopify_api_key"
            :disabled="shopify_details_saved || !from_setup"
            required
          />
        </div>

        <div class="form-group mb-2" v-if="!is_shopify_public_app_for_company">
          <label> {{tlt('shopify_setting_api_secret_label')}}: </label>
          <input
            class="form-control"
            type="text"
            v-model="shopify_secret_key"
            :disabled="shopify_details_saved || !from_setup"
            required
          />
        </div>
        ...
        <div class="btn-wrap text-left mt-0 pt-0 border-top-0">
          ...
          >
           ...
           <button
            ...
            v-if="!shopify_details_saved && is_shopify_public_app_for_company"
          >
            {{tlt('shopify_automation_process')}}
          </button>
        </div>
      </form>
    </ol>
    <div>
      <h6 v-if="shopify_details_saved" style="width:100%">
        {{tlt('shopify_setting_sync_status_finish_label')}}:
        <strong style="color: #273679">
          {{ shopify_sync_status }}
          <i
            class="fa fa-spinner fa-spin"
        ...
        <div class="modal-content">
          <div class="modal-body">
            ...
            <div class="card-body" style="padding: 0px 10px">
              <ul>
                <li>
                  <h5>{{tlt('shopify_help_shop_name_label')}}</h5>
                  <p class="mb-0" style="font-weight: 500;">{{tlt('shopify_help_follow_steps_label')}}</p>
                  <ol style="padding-left: 1.2rem">
                    <li>{{tlt('shopify_help_first_instruct_one')}}</li>
                    <li>
                      {{tlt('shopify_help_first_instruct_two')}}
                    </li>
                  </ol>
                </li>
                <li style="padding-top: 10px;">
                  <h5>{{tlt('shopify_help_second_instruct_header')}}</h5>
                  <p class="mb-0" style="font-weight: 500;">{{tlt('shopify_help_second_instruct_label')}}</p>
                  <ol style="padding-left: 1.2rem">
                    <li>
                      {{tlt('shopify_help_second_instruct_one')}}
                      <a
                        href="https://accounts.shopify.com/store-login?redirect=apps%3Fshpxid%3Dedb0847d-26EE-4B99-9D84-A1207E43E02F"
                        target="blank"
                        >{{tlt('shopify_help_second_instruct_two')}}</a
                      >
                    </li>
                    <li>{{tlt('shopify_help_second_instruct_three')}}</li>
                    <li>{{tlt('shopify_help_second_instruct_four')}}.</li>
                    <li>{{tlt('shopify_help_second_instruct_five')}}.</li>
                    <li>{{tlt('shopify_help_second_instruct_six')}}.</li>
                    <li>{{tlt('shopify_help_second_instruct_seven')}}</li>
                    <li>{{tlt('shopify_help_second_instruct_eight')}}</li>
                    <li>{{tlt('shopify_help_third_instruct_label')}}
                      <ul>
                        <li>- {{tlt('shopify_help_third_instruct_one')}}</li>
                        <li>- {{tlt('shopify_help_third_instruct_two')}}</li>
                        <li>- {{tlt('shopify_help_third_instruct_three')}}</li>
                        <li>- {{tlt('shopify_help_third_instruct_four')}}</li>
                      </ul>
                    </li>
                    <li>{{tlt('shopify_help_third_instruct_five')}}.</li>
                    <li>{{tlt('shopify_help_third_instruct_six')}}.</li>
                      <li> {{tlt('shopify_help_third_instruct_seven')}}.
...
export default {
  ...
  watch: {
    shopify_details_saved() {
      bus.$emit("shopify_details_saved", this.shopify_details_saved);
    },
  },
  components: {
  ...
  computed: {
    ...
    shopify_store_details_text() {
      if (this.shopify_details_saved) {
        return  this.tlt('shopify_details_text_saved');
      } else if(!this.shopify_details_saved && !this.is_shopify_public_app_for_company) {
        return  this.tlt('shopify_details_text');
      } else if(!this.shopify_details_saved && this.is_shopify_public_app_for_company) {
        return  this.tlt('shopify_details_store_saved');
      }
    },
    shopify_sync_status() {
      var status;
      if ((this.shopify_sync_current_step < this.shopify_sync_steps.length) && this.is_sync_finished === false) {
        status = "In-Progress";
        this.$emit("shopify_sync_status", status);
        bus.$emit("shopify_sync_status", status);
        return "In-Progress";
      } else {
  ...
  methods: {
    set_interval_check(){
      setInterval(this.check_shopify_status ,60000);
    },
    shopify_help(){
      let routeData = this.$router.resolve({name: 'shopifyhelp'});
        ...
        .then((response) => {
          ...
          if (response.data.message.MSG_CODE === this.api_status_code.DATA_AVAILABLE.MSG_CODE) {
            if ("shopify_details" in response.data.data) {
              this.shopify_shop_name =
                response.data.data.shopify_details.shopify_shop_name;
              this.is_shopify_public_app_for_company = response.data.data.shopify_details.shopify_public_app_installed;
              if(Boolean(response.data.data.shopify_details.shopify_public_app_installed) === false){
                this.shopify_secret_key =
                  response.data.data.shopify_details.shopify_secret_key;
                this.shopify_api_key =
                  response.data.data.shopify_details.shopify_api_key;
              }
              // this.shopify_details_saved = true;
              this.is_integrated = true;
              this.$root.$emit('framework_integrated',this.is_integrated, "shopify");
            }
    ...
    submit_shopify_form() {
      var domain_name = ".myshopify.com";
      if (!this.shopify_shop_name.includes(domain_name) && !this.is_shopify_public_app_for_company) {
        Swal.fire({
          text:
            this.tlt('shopify_enter_domain_name'),
          toast: true,
          position: "top-end",
          type: "warning",
          showConfirmButton: false,
      ...
      var shop_name;
      if(!this.is_shopify_public_app_for_company){
        shop_name = this.shopify_shop_name.substr(
          0,
          this.shopify_shop_name.indexOf(".")
        );
      } else {
        shop_name = this.shopify_shop_name;
      }
      Swal.fire({
        text: this.tlt('shopify_submit_your_details'),
        type: "info",
        toast: true,
        showConfirmButton: false,
      });
      axios
        .post(
          api_calls.chatbot_integration_details(),
          {
            ...
            shopify_api_key: (this.is_shopify_public_app_for_company) ? null : this.shopify_api_key,
            shopify_secret_key: (this.is_shopify_public_app_for_company) ? null : this.shopify_secret_key,
            template_id: this.retail_bot_template_id,
            shopify_public_app_installed: this.is_shopify_public_app_for_company
          },
          {
            headers: {
              Authorization: `Bearer ${this.$session.get("at")}`,
        ...
        .then((response) => {
          if (response.data.message.MSG_CODE === this.api_status_code.SUCCESSFUL_DB_QUERY.MSG_CODE) {
            this.$session.set("retail_web_framework", "shopify");
            $("#widget_code_modal").modal("hide");
            Swal.fire({
              title: this.tlt('shopify_credential_verified'),
              text:
                this.tlt('shopify_product_sync_process'),
              type: "success",
              timer: 7000,
            }).then(() => {
              this.get_integration_details();
            ...
            Swal.fire({
              title: this.tlt('woo_issue_with_credential'),
              text:
                this.tlt('shopify_issue_with_credential2'),
              type: "error",
            });
          } else if (response.data.message.MSG_CODE === this.api_status_code.INVALID_SHOPIFY_API_KEY_AND_TOKEN.MSG_CODE) {
            Swal.fire({
              title: this.tlt('shopify_oops2'),
              text:
                this.tlt('shopify_issue_with_credential'),
              type: "error",
            });
          }  else if(response.data.message.MSG_CODE === this.api_status_code.SHOP_NO_PRODUCTS.MSG_CODE) {
            Swal.fire({
              title: this.tlt('shopify_oops3'),
              text:
                this.tlt('shopify_one_product_ahead'),
              type: "error",
            });
          } else {
            Swal.fire({
              title: this.tlt('magento_error_occurred'),
              type: "error",
              toast: true,
              position: "top-end",
            });
        ...
        .catch((e) => {
          Swal.fire({
            title: this.tlt('magento_error_occurred2'),
            type: "error",
          });
          if (
            e.response.status === 410 ||
    ...
    edit_shopify_details() {
      this.shopify_details_saved = !this.shopify_details_saved;
    },
    check_shopify_status() {
      this.big_spinner = true;
      ...
      axios
        ...
        .then((response) => {
          ...
          this.is_sync_finished = false;
          this.shopify_sync_current_step = 0;
          if (response.data.message.MSG_CODE === this.api_status_code.SENDING_EMAIL.MSG_CODE) {
           this.shopify_details_saved = true;
            // if (response.data.data.Completed == true) {
            //   for (let i = 0; i <= 5; i++) {
            //     setTimeout(() => {
            //       this.shopify_sync_current_step += 1;
            //     }, 84 * i * 10);
            //   }
            //   // this.shopify_sync_current_step = 4;
            // } else {
              if (
                ...
                // this.shopify_sync_current_step = 2;
                for (let i = 0; i <= 2; i++) {
                  setTimeout(() => {
                    this.shopify_sync_current_step += 1;
                  }, 84 * i * 10);
                ...
                // this.shopify_sync_current_step = 3;
              } else if (response.data.data.Status == "Completed" && response.data.data.Completed == true ) {
                this.is_sync_finished = true;
                // var status = response.data.data.Status;
                // this.$root.$emit("shopify_sync_status", status);
                for (let i = 0; i <= 5; i++) {
                  setTimeout(() => {
                    this.shopify_sync_current_step += 1;
                  }, 84 * i * 10);
            ...
            this.shopify_details_saved = false;    
          }else{
            Swal({
              title:response.data.message.MSG_CODE,
              text:response.data.message.MSG,
