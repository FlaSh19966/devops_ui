<template>
  <div :style="setup_css" class="dash-modal">
    <div class="modal-head align-items-center m-0">
      ...
      <h3 style="text-align:center" v-if="from_setup == false">{{tlt('magento_setting_header')}}</h3>
      <button
    ...
    <div class="modal-head d-flex align-items-center m-0">
      <h5 v-if="magento_details_saved" style="width:100%">
        {{tlt('magento_setting_sync_status_label')}}:
        <strong style="color: #273679">
          {{ magento_sync_status }}
          <i
            class="fa fa-spinner fa-spin"
            v-show="magento_sync_status === 'In-Progress'"
          ></i>
        </strong>
        <i class="fa fa-refresh"></i>
        <span
          v-html="refresh_icon_svg"
          @click="check_magento_status"
    ...
    <div class="slide-right" v-if="magento_details_saved">
      <h5 v-show="magento_sync_status === 'In-Progress' && from_setup"
        style="font-weight: 500;">
        {{tlt('magento_setting_setup_process_label')}}.
      </h5>
    </div>
    <div class="progress-sync" v-if="magento_details_saved">
      <step-progress
        :steps="magento_sync_steps"
        :current-step="magento_sync_current_step"
    ...
    <h5>
      ...
      <!-- <span
        ...
        title="Edit Details"
      ></span> -->
      <span @click="magento_help" v-if="!magento_details_saved" 
    ...
    <ol class="pl-0 mt-2">
      <form @submit.prevent="submit_magento_form">
        <div class="form-group mb-2">
          <label> {{tlt('magento_setting_shop_url_label')}}: </label>
          <input
            class="form-control"
            type="text"
            placeholder="Ex. https://mymagento.com"
            v-model="magento_shop_url"
            :disabled="magento_details_saved || !from_setup"
            required
          />
        </div>

        <div class="form-group mb-2">
          <label> {{tlt('magento_setting_access_token_label')}}: </label>
          <input
            class="form-control"
            type="text"
            v-model="magento_access_token"
            :disabled="magento_details_saved || !from_setup"
            required
          />
        </div>
    ...
    <div>
      <h6 v-if="magento_details_saved" style="width:100%">
        {{tlt('magento_setting_sync_status_finish_label')}}:
        <strong style="color: #273679">
          {{ magento_sync_status }}
          <i
            class="fa fa-spinner fa-spin"
...
export default {
  ...
  watch: {
    magento_details_saved() {
      bus.$emit("magento_details_saved", this.magento_details_saved);
    },
  },
  components: {
  ...
  computed: {
    ...
    magento_store_details_text() {
      if (this.magento_details_saved) {
        return this.tlt('magento_details_text');
      } else {
        return this.tlt('enter_details_text');
      }
    },
    magento_sync_status() {
      var status;
      if (this.magento_sync_current_step < this.magento_sync_steps.length) {
        status = "In-Progress";
        this.$emit("magento_sync_status", status);
        bus.$emit("magento_sync_status", status);
        return this.tlt('in_progress_msg');
      } else {
        status = "Sync Finished";
        this.$emit("magento_sync_status", status);
        bus.$emit("magento_sync_status", status);
        return this.tlt('sync_finished_msg');
      }
    },
  },
  mounted() {
    this.check_magento_status();
  ...
  methods: {
    set_interval_check(){
      setInterval(this.check_magento_status ,60000);
    },
    magento_help(){
      let routeData = this.$router.resolve({name: 'magentohelp'});
      ...
      axios
        ...
        .then((response) => {
          ...
          if (response.data.message.MSG_CODE === this.api_status_code.DATA_AVAILABLE.MSG_CODE) {
            if ("magento_details" in response.data.data) {
              this.magento_shop_url =
                response.data.data.magento_details.magento_shop_url;
              this.magento_access_token =
                response.data.data.magento_details.magento_access_token;
              this.magento_details_saved = true;
              this.is_integrated = true;
              this.$root.$emit('framework_integrated',this.is_integrated, "magento");
            }
    ...
    submit_magento_form() {
      ...
      if (!this.magento_shop_url.includes(domain_name)) {
        Swal.fire({
          text:
            this.tlt('magento_enter_domain_name'),
          toast: true,
          position: "top-end",
          type: "warning",
          showConfirmButton: false,
      ...
      if (this.magento_access_token == "") {
        Swal.fire({
          text:
            this.tlt('magento_enter_access_token'),
          toast: true,
          position: "top-end",
          type: "warning",
          showConfirmButton: false,
      ...
      Swal.fire({
        text: this.tlt('magento_submit_your_details'),
        type: "info",
        toast: true,
        showConfirmButton: false,
      });
      axios
        .post(
          api_calls.chatbot_integration_details(),
          {
            ...
            is_magento: true,
            is_save: true,
            magento_shop_url: this.magento_shop_url,
            magento_access_token: this.magento_access_token,
        ...
        .then((response) => {
          if (response.data.message.MSG_CODE === this.api_status_code.SUCCESSFUL_DB_QUERY.MSG_CODE) {
            this.$session.set("retail_web_framework", "magento");
            $("#widget_code_modal").modal("hide");
            Swal.fire({
              title: this.tlt('magento_credential_verified'),
              text:
                this.tlt('magento_product_sync_process'),
              type: "success",
              timer: 7000,
            }).then(() => {
              this.get_integration_details();
            ...
            Swal.fire({
              title: this.tlt('magento_oops1'),
              text:
                this.tlt('magento_issue_with_credential'),
              type: "error",
            });
            ...
            Swal.fire({
              title: this.tlt('magento_oops1'),
              text:
                this.tlt('magento_one_product_ahead'),
              type: "error",
            });
          } else if (response.data.message.MSG_CODE === this.api_status_code.MAGENTO_INVALID_ACCESS_TOKEN.MSG_CODE) {
            Swal.fire({
              title: this.tlt('magento_oops1'),
              text:
                this.tlt('magento_access_token'),
              type: "error",
            });
          } else {
            Swal.fire({
