<template>
  <div :style="setup_css" class="dash-modal">
    <div class="modal-head align-items-center m-0">
      ...
      <h3 style="text-align:center" v-if="from_setup == false">{{tlt('woo_setting_header')}}</h3>
      <button
    ...
    <div class="modal-head d-flex align-items-center m-0">
      <h5 v-if="woocommerce_details_saved" style="width:100%">
        {{tlt('woo_setting_sync_status_label')}}:
        <strong style="color: #273679">
          {{ woocommerce_sync_status }}
          <i
            class="fa fa-spinner fa-spin"
            v-show="woocommerce_sync_status === 'In-Progress'"
          ></i>
        </strong>
        <i class="fa fa-refresh"></i>
        <span
          v-html="refresh_icon_svg"
          @click="check_woocommerce_status"
    ...
    <div class="slide-right" v-if="woocommerce_details_saved">
      <h5 v-show="woocommerce_sync_status === 'In-Progress' && from_setup"
        style="font-weight: 500;">
        {{tlt('woo_setting_setup_process_label')}}.
      </h5>
    </div>
    <div class="progress-sync" v-if="woocommerce_details_saved">
      <step-progress
        :steps="woocommerce_sync_steps"
        :current-step="woocommerce_sync_current_step"
    ...
    <h5>
      {{ woocommerce_store_details_text }}
      <!-- <span
        @click="edit_woocommerce_details"
        v-html="pencil_svg"
        class="ml-auto"
        style="cursor: pointer; float: right"
        title="Edit Details"
      ></span> -->
      <span @click="woocommerce_help" v-if="!woocommerce_details_saved" 
    ...
    <ol class="pl-0 mt-2">
      <form @submit.prevent="submit_woocommerce_form">
        <div class="form-group mb-2">
          <label> {{tlt('woo_setting_shop_url_label')}}: </label>
          <input
            class="form-control"
            type="text"
            placeholder="Ex. https://mywoocommerce.com"
            v-model="woocommerce_shop_url"
            :disabled="woocommerce_details_saved || !from_setup"
            required
          />
        </div>

        <div class="form-group mb-2">
          <label> {{tlt('woo_setting_consumer_key_label')}}: </label>
          <input
            class="form-control"
            type="text"
            v-model="woocommerce_consumer_key"
            :disabled="woocommerce_details_saved || !from_setup"
            required
          />
        </div>
        ...
        <div class="form-group mb-2">
          <label> {{tlt('woo_setting_consumer_secret_label')}}: </label>
          <input
            class="form-control"
            type="text"
            v-model="woocommerce_consumer_secret"
            :disabled="woocommerce_details_saved || !from_setup"
            required
          />
        </div>
        ...
        <div class="btn-wrap text-left mt-0 pt-0 border-top-0">
          <button
            ...
            data-toggle="tooltip"
            data-placement="top"
            v-if="!woocommerce_details_saved"
            :disabled="!from_setup"
          >
            {{tlt('woo_save_btn')}}
          </button>
    ...
    <div>
      <h6 v-if="woocommerce_details_saved" style="width:100%">
        {{tlt('woo_setting_sync_status_finish_label')}}:
        <strong style="color: #273679">
          {{ woocommerce_sync_status }}
          <i
            class="fa fa-spinner fa-spin"
...
export default {
  ...
  data() {
    return {
      ...
      woocommerce_details_saved: false,
      pencil_svg:
      ...
      woocommerce_sync_current_step: 0,
      refresh_icon_svg:
  ...
  watch: {
    woocommerce_details_saved() {
      bus.$emit("woocommerce_details_saved", this.woocommerce_details_saved);
    },
  },
  components: {
  ...
  computed: {
    ...
    woocommerce_store_details_text() {
      if (this.woocommerce_details_saved) {
        return this.tlt('woocommerce_details_text');
      } else {
        return this.tlt('woocommerce_enter_details_text');
      }
    },
    woocommerce_sync_status() {
      var status;
      if (this.woocommerce_sync_current_step < this.woocommerce_sync_steps.length) {
        status = "In-Progress";
        this.$emit("woocommerce_sync_status", status);
        bus.$emit("woocommerce_sync_status", status);
        return "In-Progress";
      } else {
  ...
  methods: {
    set_interval_check(){
      setInterval(this.check_woocommerce_status ,60000);
    },
    woocommerce_help(){
      let routeData = this.$router.resolve({name: 'woocommercehelp'});
      ...
      axios
        ...
        .then((response) => {
          ...
          if (response.data.message.MSG_CODE === this.api_status_code.DATA_AVAILABLE.MSG_CODE) {
            if ("woocommerce_details" in response.data.data) {
              this.woocommerce_shop_url =
                response.data.data.woocommerce_details.woocommerce_shop_url;
              this.woocommerce_consumer_secret =
                response.data.data.woocommerce_details.woocommerce_consumer_secret;
              this.woocommerce_consumer_key =
                response.data.data.woocommerce_details.woocommerce_consumer_key;
              this.woocommerce_details_saved = true;
              this.is_integrated = true;
              this.$root.$emit('framework_integrated',this.is_integrated, "woocommerce");
            }
    ...
    submit_woocommerce_form() {
      var domain_name = "https://";
      if (!this.woocommerce_shop_url.includes(domain_name)) {
        Swal.fire({
          text:
            this.tlt('woo_enter_domain_name'),
          toast: true,
          position: "top-end",
          type: "warning",
          showConfirmButton: false,
      ...
      if (this.woocommerce_consumer_key == "" || this.woocommerce_consumer_secret == "") {
        Swal.fire({
          text:
            this.tlt('woo_consumer_key'),
          toast: true,
          position: "top-end",
          type: "warning",
          showConfirmButton: false,
      ...
      Swal.fire({
        text: this.tlt('woo_submit_your_details'),
        type: "info",
        toast: true,
        showConfirmButton: false,
      });
      axios
        ...
        .then((response) => {
          if (response.data.message.MSG_CODE === this.api_status_code.SUCCESSFUL_DB_QUERY.MSG_CODE) {
            this.$session.set("retail_web_framework", "woocommerce");
            $("#widget_code_modal").modal("hide");
            Swal.fire({
              title: this.tlt('woo_credential_verified'),
              text:
                this.tlt('woo_product_sync_process'),
              type: "success",
              timer: 7000,
            }).then(() => {
              this.get_integration_details();
            ...
            Swal.fire({
              title: this.tlt('woo_oops1'),
              text:
                this.tlt('woo_issue_with_credential'),
              type: "error",
            });
          } else if (response.data.message.MSG_CODE === this.api_status_code.WOOCOMMERCE_INVALID_CONSUMER_SECRET.MSG_CODE) {
            Swal.fire({
              title: this.tlt('woo_oops1'),
              text:
                this.tlt('woo_issue_with_credential2'),
              type: "error",
            });
          } else if (response.data.message.MSG_CODE === this.api_status_code.WOOCOMMERCE_INVALID_CONSUMER_KEY.MSG_CODE) {
            Swal.fire({
              title: this.tlt('woo_oops1'),
              text:
                this.tlt('woo_issue_with_credential3'),
              type: "error",
            });
          } 
          else if(response.data.message.MSG_CODE === this.api_status_code.SHOP_NO_PRODUCTS.MSG_CODE) {
            Swal.fire({
              title: this.tlt('woo_oops4'),
              text:
                this.tlt('woo_one_product_ahead'),
              type: "error",
            });
          } else {
            Swal.fire({
              title: this.tlt('woo_error_occurred'),
              type: "error",
              toast: true,
              position: "top-end",
            });
        ...
        .catch((e) => {
          Swal.fire({
            title: this.tlt('woo_error_occurred2'),
            type: "error",
          });
          if (
            e.response.status === 410 ||
